<?php
/**
 * Populate Products - Fixed for Existing Database Schema
 */
...
try {
    ...
    echo "<h2>🚀 Populating Products Database (Fixed Version)...</h2>";
    ...
    // Check if image_url column exists, if not use a different approach
    ...
    $baseColumns = ['vendor_id', 'name', 'description', 'short_description', 'category', 'price', 'stock_quantity', 'status', 'featured'];
    $columns = $baseColumns;
    ...
    echo "<p>📝 Using SQL: " . htmlspecialchars($sql) . "</p>";
    ...
    foreach ($sampleProducts as $product) {
        ...
        // Remove the NOW() placeholder since it's not a parameter
        $actualSql = str_replace(', NOW()', ', NOW()', $sql);
        if ($hasCreatedAt && !$hasImageUrl) {
            $actualSql = str_replace('VALUES (' . implode(', ', array_slice($placeholders, 0, -1)) . ', NOW())', 
                                   'VALUES (' . implode(', ', array_slice($placeholders, 0, -1)) . ', NOW())', $sql);
        }
        
        try {
            $result = $db->query($actualSql, $params);
            if ($result) {
                $count++;
                echo "<p>✅ Added: " . htmlspecialchars($product['name']) . "</p>";
            }
        } catch (Exception $e) {
            echo "<p>❌ Failed to add " . htmlspecialchars($product['name']) . ": " . htmlspecialchars($e->getMessage()) . "</p>";
        }
    }
